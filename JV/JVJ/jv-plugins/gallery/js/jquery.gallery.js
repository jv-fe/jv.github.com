  ...
  JV.gallery.prototype = {
    ...
    loadItem: function(nodeId) {
        ...
        if (oldIndex != newIndex || newIndex==0) {//modify by jay
            ...
            self.nudgeScrollContent((newIndex + 1) * self.THUMBNAIL_HEIGHT, oldIndex, newIndex, self.THUMBNAIL_HEIGHT);
                ...
                self.currentImage.hide();
                var thObj=$("#thumbnail_"+nodeId),
                    large_img=thObj.attr("data-large"),
                    title=thObj.data("title"),
                    date=thObj.data("date"),
                    full=thObj.data("full"),
                    keywords=thObj.data("keywords");      
                              
                self.setImageProperty(title,date,full,keywords,nodeId)    
    ...
    thumbnailsHandle:function(){
        ...
        self.thumbnails.bind("click",function(){
            var t=$(this),
                id=t.attr("id").replace("thumbnail_",""),
                title=t.data("title"),
                date=t.data("date"),
                full=t.data("full"),
            ...
            self.loadItem(id);
            self.setImageProperty(title,date,full,keywords,id);
        })
    },
    setImageProperty:function(title,date,full,keywords,id){
        var self=this;
        var area=$("#media-meta-data"),
            title_t=$("#item-title",area),
            date_t=$(".date",area),
            full_t=$(".dt-downloads a",area),
            screen_p=$(".screen",area);

        title_t.text(title);
        date_t.text(date);
        full_t.attr("href",full);
        if(self.cfg.data.resolutionList){
            var cur_paper=self.cfg.data.resolutionList[id],
                wallpapers=[];
            for (var i in cur_paper){
                var scr='<a target="_blank" href="'+cur_paper[i]+'" class="format">'+i+'</a>';
                wallpapers.push(scr);
            }
            screen_p.html(wallpapers.join("，"));
        }    
